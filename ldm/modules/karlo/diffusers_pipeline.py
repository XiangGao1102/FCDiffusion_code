...
class UnCLIPPipeline(DiffusionPipeline):
    ...
    def enable_sequential_cpu_offload(self, gpu_id=0):
        r"""
        Offloads all configs to CPU using accelerate, significantly reducing memory usage. When called, the pipeline's
        configs have their state dicts saved to CPU and then are moved to a `torch.device('meta') and loaded to GPU only
    ...
    def _execution_device(self):
        r"""
        Returns the device on which the pipeline's configs will be executed. After calling
